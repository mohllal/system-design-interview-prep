...
A CDN is a poor fit as the *only* scale story for highly personalized, write-heavy, or strongly consistent reads. Those still need origin design, the CDN helps the cacheable and connection-heavy parts.
...
- **Anycast**: the same IP is announced from many PoPs. BGP sends the client to a nearby announcement. Fast failover, less DNS TTLs to wait out, but routing is "BGP-near," not always "user-near."
...
Many providers combine both. The interview point is the same as [global load balancing](./13-load-balancing.md): you are choosing a **user → region** map, then a **region → origin** map, and those maps have different failure modes.
...
- A CDN is a **global reverse-proxy cache**. Hits cut latency and origin load and misses still help via TLS and connection reuse.
